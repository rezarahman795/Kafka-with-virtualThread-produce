...
@Configuration
@Service
@Qualifier("kirimKecamatanProducer")
public class MasterKecamatanProduce {
    @Value("${spring.kafka.producer.bootstrap-servers}")
    private String kafkaServer;

    @Value("${kafka.topic.kecamatan}")
    private String kafkaTopic;

    @Autowired
    private final TransDao dataTrans;

    private Producer<String, KirimMasterKecamatanDTO> producer;

    private ExecutorService executorService;
    private static final int RETRY_LIMIT = 5;
    private static final Logger log = LoggerFactory.getLogger(MasterKecamatanProduce.class);

    public MasterKecamatanProduce(TransDao dataTrans) {
    ...
    public void init() {
        try {
            ...
            props.put(ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG, MasterKecamatanSerializer.class.getName());
    ...
    public void sendKafkaKirimMasterkecamatan(List<KirimMasterKecamatanDTO> data) {
        log.info("Sending batch of {} records to Kafka topic: {}", data.size(), kafkaTopic);

        for (KirimMasterKecamatanDTO dataKafka : data) {
            String key = dataKafka.getIdTransaksi();
            ProducerRecord<String, KirimMasterKecamatanDTO> record = new ProducerRecord<>(kafkaTopic, key, dataKafka);
            ...
            executorService.submit(() -> {
                ...
                while (retry < RETRY_LIMIT) {
                    try {
                        ...
                        dataTrans.setDataKirimMasterKecamatan(key, 1, "BERHASIL KIRIM");
                        break;
                    } catch (ExecutionException | InterruptedException e) {
                        retry++;
                        log.error("Retry {}/{} failed to send message: {}", retry, RETRY_LIMIT, e.getMessage(), e);
                        if (retry == RETRY_LIMIT) {
                            dataTrans.setDataKirimMasterKecamatan(key, 99, "GAGAL KIRIM");
                        }
                    }
                }
            });
