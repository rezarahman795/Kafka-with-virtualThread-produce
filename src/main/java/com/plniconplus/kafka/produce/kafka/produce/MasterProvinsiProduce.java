...
@Configuration
@Service
@Qualifier("kirimProvinsiProducer")
public class MasterProvinsiProduce {
    @Value("${spring.kafka.producer.bootstrap-servers}")
    private String kafkaServer;

    @Value("${kafka.topic.provinsi}")
    private String kafkaTopic;

    @Autowired
    private final TransDao dataTrans;

    private Producer<String, KirimMasterProvinsiDTO> producer;

    private ExecutorService executorService;
    private static final int RETRY_LIMIT = 5;
    private static final Logger log = LoggerFactory.getLogger(MasterProvinsiProduce.class);

    public MasterProvinsiProduce(TransDao dataTrans) {
    ...
    public void init() {
        try {
            ...
            props.put(ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG, MasterProvinsiSerializer.class.getName());
    ...
    public void sendKafkaKirimMasterProvinsi(List<KirimMasterProvinsiDTO> data) {
        log.info("Sending batch of {} records to Kafka topic: {}", data.size(), kafkaTopic);

        for (KirimMasterProvinsiDTO dataKafka : data) {
            String key = dataKafka.getIdTransaksi();
            ProducerRecord<String, KirimMasterProvinsiDTO> record = new ProducerRecord<>(kafkaTopic, key, dataKafka);
            ...
            executorService.submit(() -> {
                ...
                while (retry < RETRY_LIMIT) {
                    try {
                        ...
                        dataTrans.setDataKirimMasterProvinsi(key, 1, "BERHASIL KIRIM");
                        break;
                    } catch (ExecutionException | InterruptedException e) {
                        retry++;
                        log.error("Retry {}/{} failed to send message: {}", retry, RETRY_LIMIT, e.getMessage(), e);
                        if (retry == RETRY_LIMIT) {
                            dataTrans.setDataKirimMasterProvinsi(key, 99, "GAGAL KIRIM");
                        }
                    }
                }
            });
